...
class Record(Base):
    ...
    # 土地移轉總面積平方公尺
    land_total_sqare_meter = Column(Numeric)
    ...
    # 建物移轉總面積平方公尺
    building_total_sqare_meter = Column(Numeric)
    ...
    # 建物現況格局-隔間
    number_of_compartment = Column(Integer)

    # 有無管理組織
    management_committee = Column(Boolean)

    # 總價元
    total_price = Column(Numeric)

    # 單價每平方公尺
    price_per_square_meter = Column(Numeric)

    # 車位類別
    parking_space_type = Column(String(32))

    # 車位移轉總面積平方公尺
    parking_space_square_meter = Column(Numeric)
